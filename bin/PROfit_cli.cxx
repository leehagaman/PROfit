...
PROpt::PROpt(int argc, char **argv) {
        ...
        surface_command->add_flag("--pull-surface", pull_surface, "After the scan, also map the strongest-pulling nuisance parameter at every grid point (pull = (best fit - prior center)/prior sigma) as a categorical surface with Delta-chi^2 contours overlaid, a la IceCube PRD 102 052009 Fig. 23. Writes <tag>_dominant_pull_surface.pdf/.txt and TH2Ds into <tag>_surf.root.");
        surface_command->add_option("--pull-surface-group", pull_surface_group, "How to colour the dominant-pull map: 'name' = one category per systematic (default), 'tag' = group systematics by their XML tag=\"...\" attribute (untagged ones keep their own name).")->check(CLI::IsMember({"name", "tag"}))->default_val("name");
        surface_command->add_flag("--pull-surface-covar", pull_surface_covar, "Let covariance-type systematics compete too, using their analytic posterior pull at each best fit: sqrt(u^T M^-1 Sigma_k M^-1 u), the square root of source k's share of the covariance chi^2 term (= |alpha| for a single-mode source). Neyman statistical variances.");
        surface_command->add_option("--pull-surface-min", pull_surface_min, "Leave grid points whose largest |pull| is below this unassigned (white). Default 0 = always assign. Useful for Asimov surfaces where pulls inside the allowed region are ~0.")->default_val(0.0f);
        surface_command->add_option("--pull-surface-from", pull_surface_from, "Skip all fitting: build the dominant-pull map from an existing <tag>_surf.root written by a previous surface run (same XML and axes). Implies --pull-surface. Output goes to <tag>_dominant_pull_surface.pdf/.txt and <tag>_dominant_pull_surf.root; the input file is not modified.");
        surface_command->add_option("--pull-surface-levels", pull_surface_levels, "Delta-chi^2 contour levels overlaid on the dominant-pull map (drawn solid, dashed, dotted, ...). Default 4.61 5.99 9.21 = 90/95/99% CL at 2 dof.");
